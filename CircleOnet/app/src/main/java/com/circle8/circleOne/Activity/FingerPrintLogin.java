    ...
    private Cipher instantiateCipher() {
        ...
        try {
           ...
           // throw new RuntimeException("Failed to instantiate Cipher class");
            ...
            mUser = mGson.fromJson(userString, UserObject.class);
            if (mUser != null) {
                loginSession.createLoginSession(mUser.getQ_id(), mUser.getProfileid(), mUser.getUserId(), mUser.getUsername(), mUser.getEmail(), mUser.getImage(), mUser.getGender(), mUser.getPassword(), mUser.getDob(), mUser.getPhone(), mUser.getConnection_Limit(), mUser.getConnection_Left());
            ...
            if (mUser != null) {
               ...
               // Toast.makeText(context, "LoggedIn Successfully..", Toast.LENGTH_LONG).show();
                ...
                if (prefs.getBoolean("firstrun", true)) {
                    ...
                    Intent userIntent = new Intent(context, DashboardActivity.class);
                    userIntent.putExtra("viewpager_position", 0);
                    startActivity(userIntent);
                    finish();
                }
