...
public class SystemUtils {
    ...
    public static boolean isAppRunningNow() {
        ...
        if (!runningTasks.isEmpty()) {
            return runningTasks.get(0).topActivity.getPackageName().equalsIgnoreCase(MyApplication.getInstance().getPackageName());
        } else {
            return false;
        }
    }
    ...
    public static Intent getPreviousIntent(Context context) {
        ActivityManager activityManager = (ActivityManager) MyApplication.getInstance().getSystemService(Context.ACTIVITY_SERVICE);
        List<ActivityManager.RunningTaskInfo> runningTasks = activityManager.getRunningTasks(Integer.MAX_VALUE);
        String ourPackageName = MyApplication.getInstance().getPackageName();
