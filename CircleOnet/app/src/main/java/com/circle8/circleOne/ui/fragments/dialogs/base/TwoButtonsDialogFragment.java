package com.circle8.circleOne.ui.fragments.dialogs.base;
...
public class TwoButtonsDialogFragment extends DialogFragment {
    ...
    public static void show(FragmentManager fm, int message, MaterialDialog.ButtonCallback callback) {
        show(fm, null, MyApplication.getInstance().getString(message), true, null, null, callback);
    }

    public static void show(FragmentManager fm, int message) {
        show(fm, null, MyApplication.getInstance().getString(message), true, null, null, null);
    }

    public static void show(FragmentManager fm, int title, int message, MaterialDialog.ButtonCallback callback) {
        show(fm, MyApplication.getInstance().getString(title), MyApplication.getInstance().getString(message), true, null, null, callback);
    }
    ...
    public static void show(FragmentManager fm, int title, int message,
        ...
        show(fm, MyApplication.getInstance().getString(title), MyApplication.getInstance().getString(message), true,
                MyApplication.getInstance().getString(positiveText), MyApplication.getInstance().getString(negativeText), callback);
    }
